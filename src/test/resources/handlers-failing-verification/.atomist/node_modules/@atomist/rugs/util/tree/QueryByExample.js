...
function queryByExampleString(g) {
    var rootExpression = typeToAddress(g);
    var isMatch = g._match && g._match;
    // We want to put these first, as it makes for more readable path expression
    // and makes tests deterministic
    var simplePredicates = "";
    var complexPredicates = "";
    for (var id in g) {
        var propOrFun = g[id];
        var value = null;
        ...
        else if (value.nodeTags && value.nodeName) {
    ...
    return new Branch(rootExpression + simplePredicates + complexPredicates, isMatch);
}
 ...
 */
function isRelevantFunction(id, f) {
 ...
 */
function isRelevantProperty(id, p) {
